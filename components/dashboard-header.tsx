...
import { Home, History, Bell, Info, Menu, User, Sparkles, Crown, Shield, Eye } from "lucide-react"
...
export function DashboardHeader({ title, children }: DashboardHeaderProps) {
    ...
    return (
        <>
            {isImpersonating && (
                <div className="bg-amber-500 text-white px-4 py-2 text-sm font-medium flex items-center justify-between animate-in slide-in-from-top">
                    <div className="flex items-center gap-2">
                        <Eye className="h-4 w-4" />
                        <span>Estás viendo la aplicación como <strong>{user?.displayName}</strong></span>
                    </div>
                    <Button
                        variant="secondary"
                        size="sm"
                        onClick={stopImpersonation}
                        className="h-7 text-xs bg-white text-amber-600 hover:bg-amber-50 border-none"
                    >
                        Salir de la vista
                    </Button>
                </div>
